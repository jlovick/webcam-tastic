...
begin
  ...
  ARGV.each do |arg|
    ...
    when /\A--brightness=([\ 0-9A-Za-z._\/]+)\Z/ then bright = $1.to_i
    when /\A--sql=((.*)+)\Z/ then sql = $1.to_s
    end
  end
 usage() unless ( outfile and camname and expname and channelname and tablename and bright)
  puts " using these options \n outfile=\"#{outfile}\",  camname=\"#{camname}\", expname=\"#{expname}\", channelname=\"#{channelname}\",  tablename=\"#{tablename}\"  bright=\"#{bright}\"  --sql=\"#{sql}\" "
  puts " system is :"
...
all_src_files = DB.from(:src_files_table).filter(:src_camera => cam)
...
#stat_entries.filter(sql)
#FasterCSV.open(outfile, "w") do |csv|
#stat_entries.each do |entry|
#
#  res = Mean_Table.filter(:id=>entry[:Table_key]).filter(:DATA <  bright).select(:DATA).single_value
...
#
#  if (res)
#    csv << [entry[:src_file_id],tt.to_s,res]
#  end
#      end
#end

  stat_entries = New_Stats.filter(:table_id => tbl).filter(:src_file_id => all_src_files ).filter(:Experiment_id => expr).filter(:Channel => chan).select(:src_file_id, :Table_key)
  dated_stats = New_Stats.join( :all_src_files, :src_file_id => all_src_files).sql
  pp dated_stats
  exit(0)
